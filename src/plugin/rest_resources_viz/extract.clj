...
(def ^:private deps '[[org.clojure/clojure "1.9.0-alpha14"]
                      ...
                      [com.elasticpath.plugins/rest-resources-viz "0.1.0-SNAPSHOT" :classifier "sources"]])

(def ^:private clojure-paths ["main/clojure"])

(defn run-extractor
  [repo-system repo-system-session project classpath-paths opts]
  (log/debugf "Options: %s" opts)

  (let [local-repo (or (.. repo-system-session getLocalRepository getBasedir)
...
(mojo/defmojo ExtractMojo
  ...
  ;; parameters
  [repo-system {:role "org.sonatype.aether.RepositorySystem"}
   repo-system-session {:defaultValue "${repositorySystemSession}"
                        :readonly true}
   ...
   debug? {:expression "${clojure.nrepl.port}"
           :defaultValue "false"
           :description "Enable extra output and data validation"}
